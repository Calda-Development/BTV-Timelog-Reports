...
export function TimelogResults({
  data,
  selectedDates,
  selectedUsers,
}: TimelogResultsProps) {
  ...
  const buildTextReport = () => {
    ...
    if (selectedDates.length === 1) {
      ...
      const endDate = new Date(
        selectedDates[selectedDates.length - 1] + "T00:00:00"
      );
  ...
  const daysWithData = selectedDates.filter(
    (date) => data.timelogGroups[date] && data.timelogGroups[date].length > 0
  );
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button onClick={() => copyToClipboard(buildTextReport())}>
        ...
        <CardContent className="border-t">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6">
            <div className="text-center">
              <div className="text-2xl font-bold text-foreground">
                {Object.keys(userEntries).length}
              </div>
              <div className="text-sm text-muted-foreground">Team Members</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-foreground">
                {Object.values(userEntries).reduce(
              ...
              <div className="text-2xl font-bold text-foreground">
                {convertTime(
                  Object.values(userEntries)
                    .flat()
                    .reduce((acc, entry) => {
                      const timeInSeconds = entry.timeSpent
                        .split(":")
          ...
          <div className="mt-6 pt-6 border-t">
            ...
            <div className="space-y-2">
              ...
              {(() => {
                const allTeamMembers = Object.values(NAME_MAPPING);
                const activeMembers = Object.keys(userEntries);
                const missingMembers = allTeamMembers.filter(
                  (displayName) => !activeMembers.includes(displayName)
                );
                const allUsersSelected =
                  selectedUsers.length === Object.keys(NAME_MAPPING).length;
                ...
                return allUsersSelected && missingMembers.length > 0 ? (
                  <div className="mt-4">
                    ...
                    {missingMembers.map((member) => (
                      <div
                        key={member}
                        className="flex justify-between items-center py-2 px-3 bg-destructive/10 border border-destructive/20 rounded-lg"
                      >
            ...
            <div className="space-y-2">
              {daysWithData.map((date) => {
                ...
                return (
                  <div
                    key={date}
                    className="flex items-center justify-between p-3 bg-background border border-[#9d6221]/20 dark:border-[#9d6221]/40 rounded-lg hover:bg-muted/30 transition-colors"
                  >
                    ...
                    <Button
                      onClick={() =>
                        copyToClipboard(buildDayReport(date), undefined, date)
                      }
                      variant="outline"
                      size="sm"
              ...
              <Button
                onClick={() =>
                  copyToClipboard(
                    entries
                      .map((entry) => {
                        const cleanedTitle = entry.issueTitle.replace(
                          /\[|\]/g,
                          ""
                        );
